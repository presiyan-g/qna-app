...
export function QuestionForm({
  ...
  return question ? (
    ...
    <CreateQuestionForm
      ...
      cadence={cadence}
      initialRemainingQuota={initialRemainingQuota ?? null}
    />
  );
}
...
function CreateQuestionForm({
  ...
  // Controlled state for fields the AI can write into
  const [prompt, setPrompt] = useState('');
  const [explanation, setExplanation] = useState('');
  const [choiceRows, setChoiceRows] = useState<QuestionFormChoice[]>(() => emptyChoices());
  ...
  const applyDraft = (draft: Draft) => {
    ...
    // AI drafts are always exactly 4 choices (Draft.choices is a 4-tuple).
    // If the user previously had 2 or 6 rows, applying overwrites to 4.
    // Choice-level imageUrls at positions 0-3 are preserved per spec §3.3.
    setChoiceRows((prev) =>
              ...
              <div className="grid grid-cols-[36px_1fr_auto] gap-2">
                <label className="flex h-11 items-center justify-center rounded-lg border border-line bg-card">
                  <input
                    ...
                    onChange={() =>
                      setChoiceRows((rows) =>
                        rows.map((r, i) => ({ ...r, isCorrect: i === index })),
                      )
                    }
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setChoiceRows((rows) =>
                      rows.map((r, i) => (i === index ? { ...r, label: val } : r)),
                    );
                  }}
